            ...
            <tbody className="divide-y divide-gray-200">
              {socketcontext?.zenList ? socketcontext.zenList.map((list: userType) =>
                <tr className='h-max w-max'>
                  ...
                  <td className="px-6 py-4 text-sm font-medium text-center whitespace-nowrap">
                    {list.active == true ? 
                    <p className='bg-green-200 text-green-600 p-1 rounded'>Active</p> :
            ...
            socketcontext?.zenList ? socketcontext.zenList.map((list: userType) => (
              <div className='h-max w-[90vw] p-2 rounded flex bg-slate-100 border-2 border-gray-200 flex-row justify-between items-center'>
                ...
                <span className=' w-[35%] flex flex-col gap-1'>
                  <p className='font-medium text-base'>{list.firstname}</p>
                  <p className='text-sm font-normal text-gray-500'>{list.zen_no}</p>
                </span>
                {
                  list.active == true ?
