...
app_1.io.on('connection', (socket) => {
    ...
    socket.on('call', (zenno, from, offer) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            sender = from;
            sendersOffer = offer;
            app_1.io.to(receiver).emit('callercalling');
        }
        catch (error) {
            console.log(error);
        }
    ...
    socket.on('recieved', () => {
        app_1.io.to(receiver).emit('recieverCall', { sendersOffer, sender });
    });
    socket.on('callrecieved', (answer) => {
        app_1.io.to(sender).emit('callaccepted', { answer, picked: true });
    });
    socket.on('done', () => { app_1.io.emit('videocall'); });
